...
MIRROR = False
DATASET_SIZE = 166 # the size of KITTI2015 Training dataset
...
def get_dataset(shuffle=True, name='*', scene='*', list_path=""):
    ...
    train_dataset = dst.take(train_size)
    val_dataset = dst.skip(train_size)
    print("Generating datasets")
    return train_dataset.batch(BATCH_SIZE, drop_remainder=True).prefetch(AUTOTUNE), val_dataset.batch(BATCH_SIZE, drop_remainder=True).prefetch(AUTOTUNE)
...
if __name__ == "__main__":
    ...
    weights_dir = 'logs/e2e/20220413-193836-real_kitti-264x648_d192_IR-0.01_ITER-21_TR-2e-05_BS-4_leastereo-supermodular'
    ...
    with mirrored_strategy.scope():
        ...
        model.load_weights(weights_dir + '/128.ckpt')
    ...
    for i in range(10):
        ...
        callbacks = [tf.keras.callbacks.TensorBoard(log_dir=log_dir), #write_graph=False, profile_batch=0),
                 ...
                 tf.keras.callbacks.ModelCheckpoint(
                     ...
                     save_freq='epoch')]
        train_dataset, val_dataset = get_dataset(shuffle=True, name=DS_NAME, scene='train-2015', list_path="")
        # val_data = get_dataset(shuffle=False, name='real_kitti', scene='validation-201*')
        training_history = model.fit(train_dataset, callbacks=callbacks, epochs=20, initial_epoch=0, validation_data=val_dataset)# ,validation_steps=int(20//BATCH_SIZE),steps_per_epoch = 41)
        avg_loss = np.average(training_history.history['loss'])
        avg_val_loss = np.average(training_history.history['val_loss'])
        mean_losses.append(avg_loss)
        mean_val_losses.append(avg_val_loss)
        print("Average test loss: ", avg_loss)
    
    for i in range(10):
        cur_loss = mean_losses[i]
        cur_val_loss = mean_val_losses[i]
        print(f"The {i}th average loss: {cur_loss}")
        print(f"The {i}th average val loss: {cur_val_loss}")
        print("--------")
