...
DISPARITIES = 192
# DS_NAME = 'sceneflow'
DS_NAME = "kitti2015first"
#DATA_DIR = "/data/hao/vision/vkitti"
DATA_DIR = "/home/yangxiao/vision/training/"
NET = 'leastereo'
CRF = True
...
def path2data(lp, rp, tl, tr=None):
    ...
    # while(offset[1] > tf.shape(images)[1] - CROPPED_H-1 or offset[2] > tf.shape(images)[2] - CROPPED_W-1):
    #     offset = tf.random.uniform(shape=[4], dtype=tf.int32, maxval=tf.int32.max) % limit
...
def get_dataset(shuffle=True, name='*', scene='*', list_path=""):
    if list_path:
        ...
        # rtp = tf.data.Dataset.list_files(DATA_DIR + f"/{name}/{scene}/frames/depth/Camera_1/*",
        #                                  False) if MIRROR else None
        lpath = tf.data.Dataset.list_files(DATA_DIR + f"/image_2/*", False)
        rpath = tf.data.Dataset.list_files(DATA_DIR + f"/image_3/*", False)
        ltp = tf.data.Dataset.list_files(DATA_DIR + f"/disp_occ_0/*", False)
        rtp = tf.data.Dataset.list_files(DATA_DIR + f"/disp_occ_1/*",
                                         False) if MIRROR else None
    dst = tf.data.Dataset.zip((lpath, rpath, ltp, rtp) if MIRROR else (lpath, rpath, ltp))
    dst = dst.cache()
    if shuffle:
...
if __name__ == "__main__":
    ...
    log_dir = '/home/yangxiao/vision/logs/' + time + f'-{DS_NAME}-{CROPPED_H}x{CROPPED_W}_d{DISPARITIES}_IR-{INFER_RATE}_ITER-{INFER_ITER}_TR-{LEARN_RT}_BS-{BATCH_SIZE}_{NET}-supermodular'
    weights_dir = 'logs/e2e/20210909-100832-sceneflow-408x720_d192_IR-0.01_ITER-21_TR-1e-05_BS-6_leastereo-supermodular/old'
    # weights_dir = "/home/yangxiao/vision/logs/20220413-165853-kitti2015first-264x648_d192_IR-0.01_ITER-21_TR-2e-05_BS-9_leastereo-supermodular"
    ...
    with mirrored_strategy.scope():
        ...
        # model.load_weights(weights_dir + '/020.ckpt')
