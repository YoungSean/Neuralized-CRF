...
DATA_DIR = "/home/yangxiao/vision/sample_kitti2015/"
NET = 'leastereo'
CRF = True
...
if __name__ == "__main__":
    ...
    log_dir = 'logs_sample0403/e2e/' + time + f'-{DS_NAME}-{CROPPED_H}x{CROPPED_W}_d{DISPARITIES}_IR-{INFER_RATE}_ITER-{INFER_ITER}_TR-{LEARN_RT}_BS-{BATCH_SIZE}_{NET}-supermodular'
    ...
    with mirrored_strategy.scope():
        ...
        model.compile(loss=loss3 if NET == 'psm' else loss, metrics=[mae], optimizer=optimizer, experimental_run_tf_function=False)
    ...
    model.fit(dataset, callbacks=callbacks, epochs=5, initial_epoch=0)
    # model.fit(dataset, epochs=3, initial_epoch=0, verbose=1)
